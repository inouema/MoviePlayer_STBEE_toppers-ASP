...
void main_task(intptr_t exinf)
{
...
#if 0 /* Display test */
	Display_Puts_If_Ex(5,5,(uint8_t*)"Hello World!",OPAQUE,COL_WHITE,COL_BLACK);
	for(;;);
#endif
	...
	if (f_mount(0, &Fatfs[0]) == FR_OK)
		Display_Puts_If_Ex(10, 10, "FatFs file mount done.", OPAQUE, COL_WHITE, COL_BLACK);
	else
		Display_Puts_If_Ex(10, 20, "FatFs file mount error.", OPAQUE, COL_RED, COL_BLACK);

	uint32_t read_n=0;
	FRESULT ret = f_open(&File[0], "test.txt", FA_OPEN_EXISTING | FA_READ);
	if (ret != FR_OK) syslog(LOG_ERROR,  "FatFs f_open() error!");

	ret = f_read(&File[0], (char *)Line, sizeof(Line)/sizeof(Line[0]), &read_n);
	if (ret != FR_OK || read_n == 0)
		syslog(LOG_ERROR,  "FatFs f_read() error!");
	else
		Display_Puts_If_Ex(10, 30, Line, OPAQUE, COL_WHITE, COL_BLACK);
